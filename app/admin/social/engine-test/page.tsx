...
export default function EngineTestPage() {
  ...
  const canGenerate = useMemo(() => {
    return !generating && referencePickKey.trim().length > 0 && prompt.trim().length > 0
  }, [generating, referencePickKey, prompt])
  ...
  const handleGenerateBoth = async () => {
    ...
    try {
      ...
      const data = await response.json()
      if (!response.ok) {
        throw new Error(data?.error || data?.details || 'Failed to run engine comparison')
      }
      ...
      setError(err instanceof Error ? err.message : 'Failed to run engine comparison')
    } finally {
      setGenerating(false)
    }
  }
    ...
    <div className={`${dmSans.className} p-6 lg:p-8 space-y-6 text-[#f7f1df]`}>
      <div>
        ...
        <p className="text-[#d7c9a6] mt-2 max-w-3xl">
          Compare the same reference photo + prompt through Gemini and gpt-image-2 side by side.
        </p>
      </div>

      <section className="rounded-2xl border border-[#7a6738]/60 bg-[#111a2f] p-5 space-y-4">
        <h2 className={`${cormorant.className} text-2xl text-[#f1d386]`}>Input</h2>
        ...
        <div>
          <label className="text-sm text-[#d7c9a6] block mb-1">Prompt</label>
          <textarea
            value={prompt}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="rounded-2xl border border-[#3d4a68] bg-[#111a2f] p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className={`${cormorant.className} text-2xl text-[#f1d386]`}>Gemini</h3>
          ...
          {geminiResult.imageUrl ? (
            ...
            <div className="h-64 rounded-lg border border-dashed border-[#3d4a68] flex items-center justify-center text-sm text-[#b9aa87]">
              {generating ? 'Running Gemini...' : 'No Gemini result yet.'}
            </div>
          )}
        </div>
        ...
        <div className="rounded-2xl border border-[#3d4a68] bg-[#111a2f] p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className={`${cormorant.className} text-2xl text-[#f1d386]`}>gpt-image-2</h3>
          ...
          {gptImage2Result.imageUrl ? (
            <img
              ...
              alt="gpt-image-2 generated result"
              className="w-full max-h-[70vh] object-contain rounded-lg border border-[#3d4a68] bg-black"
            />
          ) : (
            <div className="h-64 rounded-lg border border-dashed border-[#3d4a68] flex items-center justify-center text-sm text-[#b9aa87]">
              {generating ? 'Running gpt-image-2...' : 'No gpt-image-2 result yet.'}
            </div>
          )}
        </div>
      </section>
