...
select
  ...
  '{}'::text[],
  '[]'::jsonb,
  'Shared generation engine for /admin/social/creation using reference_previous identity chaining for follow-up slides.',
  true
where not exists (
  select 1
  from post_templates
  where name = 'Creation Engine (Previous Identity)'
);
